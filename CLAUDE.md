...
| Phase 1+ key names (LOCKED) | docs/00_architecture/SCHEMA_CONTRACT.md |
...
| FilterNode or ScoringNode | `docs/00_architecture/POINT_2_LANGGRAPH_DESIGN_v2_2.md` |
| Database schema or models | `docs/00_architecture/POINT_3_DATABASE_SCHEMA_v1_5.md` |
...
| Alembic | Only method for generating schema changes. Migrations generated via --autogenerate, then manually curated. HOWEVER: alembic upgrade head cannot run against Supabase due to NullPool + asyncpg constraint. Apply migrations via Supabase SQL Editor. See logs/supabase-setup-log-2026-04-20.md. |
...
| Step 4 Preferences screen | New onboarding screen added: frontend/lib/screens/onboarding/preferences_screen.dart. Collects budget_per_semester, transport_willing, home_zone, career_goal. Submits to POST /api/v1/profile/preferences. Assessment Complete → Preferences (pushNamed, preserves back-stack) → Chat (pushNamedAndRemoveUntil from Preferences on submit). Splash routes assessment_complete stage directly to /chat for returning users. |
...
| University cards emission | rich_ui university_card events emitted on get_recommendation, follow_up, AND profile_update intents. profile_update emits cards when route_after_profiler auto-reruns the pipeline. roadmap_timeline only emitted on get_recommendation. |
...
| route_after_profiler | Conditional edge on profiler node. profile_update + profiling_complete=True → filter_node (pipeline reruns automatically). All other cases → END. Implemented in core_graph.py. |
| profile_update auto-rerun | When student updates any constraint mid-chat with profiling_complete=True, full pipeline reruns automatically. Student sees updated cards immediately. Prevents stale recommendations on app restart. |
| trigger field values | _write_recommendation() stores: "initial" (first run), "profile_update" (auto-rerun via route_after_profiler), "manual_rerun" (explicit request). All three active — profile_update is not dead code. |
| LLM retry on 429 | _invoke_with_retry() in explanation_node.py, answer_node.py, profiler.py. 7 call sites. Wait: 30s→60s→90s. ResourceExhausted retried only. supervisor.py excluded — fail-fast correct for intent classification. Sprint 4: replace ResourceExhausted with anthropic.RateLimitError. |
| GET /profile/recommendations | Returns most recent roadmap_snapshot from DB. Reads JSONB directly (not individual fields). Orders by run_timestamp DESC. Returns empty list not null/404. Path: GET /api/v1/profile/recommendations. |
| roadmap_snapshot completeness | _write_recommendation() stores complete 20-field university cards via _build_university_card(). GET /profile/recommendations now returns full Flutter-renderable cards. |
| FLAG_DESCRIPTIONS additions | explanation_node.py: "eligibility_contact_university" and "policy_unconfirmed" added. Both handle uncertain eligibility states. Sit adjacent in dict. |
| JWT expiry | JWT_EXPIRY_MINUTES = 10080 (7 days). Render env var must be 10080. DO NOT revert to 60 minutes. |
| Phase 0b complete | Six new student_profiles columns added via migration ab05dd3617b7: aptitude_scores (JSONB), caas_scores (JSONB), vna_scores (JSONB), family_context (JSONB), prestige_preference (FLOAT default 5.0), misc_assessment_scores (JSONB). kcis_scores deferred to Phase 1C. All key names locked in SCHEMA_CONTRACT.md. |
| Marks-first onboarding order | FINAL architectural decision (Architecture Reference v2.1 §11.3). Implementation DEFERRED to Phase 1C. Current RIASEC-first flow is INTERIM only. FRONTEND_SCREEN_CONTRACTS.md will be updated at Phase 1C start. |
| SCHEMA_CONTRACT.md | Authoritative for all new field key names in Phase 1+. Covers: aptitude_scores, caas_scores, vna_scores, family_context, misc_assessment_scores, prestige_preference, kcis_scores, KCIS sub-scale strings, affinity_matrix new fields, lag_model new fields, universities.json new fields, assessment question sub_category values, Step 4 dropdown stored values. Read before any Phase 1+ implementation. |
| curriculum_level ORM gap | Column exists in Supabase student_profiles but missing from profile.py ORM. No migration needed — column already in DB. Add curriculum_level = Column(String) to profile.py in a standalone cleanup session. |
| Flutter StudentProfile model pending | frontend/lib/models/student_profile.dart does not include the 6 new Phase 0b fields. Update when Phase 1C frontend work begins — not before. |
| Assessment stack locked | Complete 7-instrument stack confirmed: RIASEC (60q), KCIS (96q, 24 sub-scales × 4), Aptitude+Belief (44q: 8 timed + 3 belief per dimension × 4), CAAS-5-SF (30q, 5 subscales incl. Cooperation), VNA original (9q, 3 dimensions: Social Status, Independence, Achievement), Hybrid CDDQ (8q: information_gap + external_conflict), Hybrid Big Five (10q: Conscientiousness 6 + Neuroticism 4). Subject capability MCQs unchanged. Total student-facing: 257 questions. All instruments have English + Roman Urdu versions. |
| Assessment tier system | Tier 1 (gate — Marks + RIASEC + Aptitude + Subject MCQs): system functions. Tier 2 (important — adds KCIS + CAAS + CDDQ + Belief questions): full counsellor adaptation. Tier 3 (accuracy — adds VNA + Big Five): enriched framing. Tiers never shown as required/optional to student. Profile Dashboard shows 38% complete framing. Tier 2 prompt fires once after turn_count > 2. |
| KCIS stability guarantee | 24 sub-scales cover every bachelor degree category in Karachi. Adding new universities requires only populating affinity_matrix rows — no new questions. Sub-scales are category-level, not degree-level. |
| PROFILER_REQUIRED_FIELDS | Locked as empty list []. ProfilerNode does NOT aggressively ping for budget/transport/zone — these come from Step 4 Preferences screen. family_context also from Step 4 — not conversational extraction. One exception: social_pressure_field asked once when family_career_expectation == "expects_specific_field". |
| Family context collection | family_career_field and family_career_expectation collected via Step 4 Preferences screen (dropdown + radio). NOT conversational ProfilerNode extraction. Stored in family_context JSONB column. social_pressure_field: one conversational question, one trigger condition, never repeated. |
| ProfilerNode tier-aware strategy | Three strategies selected each turn from state: tier1_compensatory (no Tier 2 data — conversational proxy questions), tier2_standard (CAAS+KCIS present — stage driven by caas_scores.concern), tier3_enriched (all tiers — adds VNA prestige derivation + Big Five framing). |
| 3D RIASEC spherical model | Upgrade from dot product to Prediger 2D projection + prestige dimension (Personal Globe Inventory). people_things and ideas_data axes added to affinity_matrix. prestige_tier (1-10) per field. Match formula: 60% geometric distance + 30% dot product + 10% prestige alignment. Differentiation + consistency scoring drives ProfilerNode strategy selection. |
| VNA instrument | Original 9-question instrument written by team. Based on Theory of Work Adjustment (Dawis & Lofquist 1964 — public domain). Three active dimensions: Social Status, Independence, Achievement. Security/Altruism/Compensation deferred to Sprint 4. Viva description: "Original VNA grounded in TWA, adapted for Karachi context." MIQ rejected: CC BY-NC license prohibits commercial use. |
| MBTI explicitly rejected | National Academy of Sciences (1991): insufficient research for career counselling. 2024 large-scale study: Big Five 2× more accurate; adding MBTI to Big Five provides ~zero incremental value. MBTI binary categories lose ~38% accuracy vs continuous Big Five. |
| Big Five selection rationale | Conscientiousness kept: strongest Big Five predictor of academic GPA (r=0.24 meta-analytic). CAAS Control covers only ~15% of Conscientiousness variance — insufficient. Neuroticism kept: predicts degree persistence in high-pressure programmes independently of CAAS Confidence (~12-20% overlap). Openness dropped: CAAS Curiosity covers ~40% adequately. Extraversion dropped: CAAS Cooperation covers interpersonal dimension. Agreeableness dropped: predicts workplace harmony not field suitability — no routing use. |
| EQ-i Youth deferred | Deferred to Sprint 4. Reinstatement trigger: Social-RIASEC degrees >30% of recommendation pool OR disproportionate mismatch in user feedback. When reinstated: Tier 2, reduces to ~42-45 questions after overlap removal with existing stack. |
| Dual dashboard architecture | Two separate data paths: Profile Dashboard (static — reads assessment scores from student_profiles directly, no nodes, only changes on retake) vs Degree Dashboard (dynamic — reads from recommendations table, changes with every pipeline run). Profile Dashboard tabs: RIASEC+KCIS, Aptitude+Belief, CAAS-5, Values, Decision Style. |
| LLM profile summary pipeline | POST /api/v1/profile/generate-summary. 7 LLM calls: 6 parallel per-module (RIASEC+KCIS, Aptitude+Belief, CAAS, VNA, CDDQ+BigFive, Marks) + 1 synthesis call (600-800 word narrative). Separate from LangGraph. Triggered once at assessment completion. Stored statically in student profile. Regenerates only on assessment retake. Reality-hit framing mandatory: marks below requirements stated directly, never softened. |
| Assessment back button lock | Within a test: free navigation between questions. After test confirmation dialog: LOCKED — no back navigation. From complete screen: forward only. Standard psychometric validity requirement — prevents response bias from students adjusting earlier answers based on later questions. |
| LLM library rule scope | langchain_google_genai rule applies ONLY to backend/app/agents/ nodes. Standalone scripts in backend/scripts/ may use google.generativeai directly when they have no LangGraph or FastAPI dependency. Script C (map_job_titles.py) is the first confirmed exempt script. |
| Rationale documentation standard | docs/rationale/ folder. One file per major subsystem or component. Written AFTER 100% complete and committed — never during or before. Architecture Chat checks and fills gaps after each completion. Files are NOT authoritative — CLAUDE.md and committed code win on any conflict. Full documentation standard is in ARCHITECTURE_CHAT_INSTRUCTIONS_v2.md and docs/rationale/README.md. Standard requires: 4-level hierarchy (system/subsystem/component/algorithm), formal algorithm specifications (name, purpose, input, output, pseudocode in formal notation, complexity, invariants, edge cases, design rationale), three diagram types per subsystem document (system context, internal component, data flow), self-contained operational reference. Trigger for LinkedIn data pipeline document: Scripts A+B+C+D all committed and verified. |
...
iq_weight in FAST universities.json was redistributed fully into
math_weight as a deliberate interim decision (May 2026). Reason:
ExplanationNode only reads five standard keys and silently ignores
iq_weight. Keeping it as a separate key would mean zero advice
generated for the IQ section — redistribution to math at least
produces directionally correct advice since analytical reasoning
correlates with mathematical ability. This was confirmed after
research into FAST NU Test structure. This redistribution must
be reverted if the answer to this question is Yes.

**If yes:**
- Revert iq_weight redistribution in universities.json for FAST
  entries (restore adv_math_weight, basic_math_weight, iq_weight
  as separate keys before normalization)
- Add iq_score to student_profiles (Phase 1C migration)
- Add iq_weight as a sixth standard entry_test key in ExplanationNode
- Add 10-12 analytical reasoning questions to assessment_questions.json if required
...
*CLAUDE.md v2.6 — May 2026 (OPEN DESIGN QUESTIONS section added; OQ-01 IQ/analytical assessment question for Phase 1C; iq_weight redistribution rationale documented)*
*CLAUDE.md v2.7 — May 2026 (LLM library rule scope clarified — langchain_google_genai applies to agents/ nodes only; google.generativeai permitted in backend/scripts/; rationale documentation rule added — sdocs/rationale/ folder, written after completion, not authoritative)*