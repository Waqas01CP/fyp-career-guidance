...
### Status: UPDATED (v2.2 — tier-aware ProfilerNode,
### route_after_profiler, 8 new AgentState fields,
### profile_update pipeline auto-rerun)
### v2.1 superseded by this file.
...
here                |  (LLM - classifies intent, writes last_intent)   |
  ...
  (if profiling complete)              |               "market_query"         |
        |                              |               "follow_up"            v
        |                              v               "clarification"     AnswerNode
        |                        ProfilerNode               |
        |                    (LLM - conversational)         v
        |                              |               AnswerNode
        |               route_after_profiler()              |
        |              /                    \               v
        |   profile_update +           everything          END
        |   profiling_complete=True    else
        |              |                    |
        |              v                    v
        |          [pipeline]              END
        |          auto-reruns
...
**route_after_profiler:** A second conditional edge on the profiler node. When `profile_update` + `profiling_complete=True`, automatically routes to `filter_node` for a fresh pipeline run. All other cases route to END. See Section 5b for full details.
...
class AgentState(TypedDict):
    ...
    # ── Assessment scores (Phase 1B/1C data, Phase 2 node reads) ─────
    aptitude_scores: dict
    # Written after aptitude assessment completed in onboarding.
    # Keys: "numerical", "spatial", "verbal", "logical" → float 0-100.
    # Read by ScoringNode for required_capability matching.
    # Empty dict {} until assessment completed (Tier 1 partial).

    caas_scores: dict
    # Written after CAAS-5-SF completed in onboarding.
    # Keys: "concern", "control", "curiosity", "confidence",
    #        "cooperation" → float 1-5 (Likert average per subscale).
    # Read by ProfilerNode to select counsellor strategy.
    # Empty dict {} until CAAS completed (Tier 2).

    kcis_scores: dict
    # Written after KCIS sub-scale assessment completed.
    # Keys: sub-scale names e.g. "mathematical_computational" → float 0-100.
    # Read by ScoringNode for within-RIASEC-type discrimination.
    # Empty dict {} until KCIS completed (Tier 2).

    vna_scores: dict
    # Written after VNA (Vocational Needs Assessment) completed.
    # Keys: "social_status", "independence", "achievement" → float 0-100.
    # Read by ScoringNode and ExplanationNode for values alignment.
    # Empty dict {} until VNA completed (Tier 3).

    family_context: dict
    # Written from Step 4 Preferences screen — NOT conversational extraction.
    # Keys: "family_career_field" (str),
    #        "family_career_expectation" (str),
    #        "social_pressure_field" (str or None).
    # family_career_expectation values:
    #   "expects_specific_field" | "general_preferences" | "fully_open"
    # social_pressure_field set by ONE ProfilerNode follow-up question,
    #   triggered ONLY when expectation == "expects_specific_field".
    # Read by ProfilerNode for family-pressure counsellor strategy.

    prestige_preference: float
    # Derived by ProfilerNode from family_context + vna_scores.
    # Scale: 0.0 (low prestige priority) to 10.0 (high).
    # High value when: family_career_field is medicine or engineering
    #   AND family_career_expectation is "expects_specific_field" or
    #   "general_preferences" AND vna_scores["social_status"] is high.
    # Read by ScoringNode for 3D match formula prestige axis.
    # Default: 5.0 (neutral) until ProfilerNode computes it.

    # ── ProfilerNode control ──────────────────────────────────────────
    tier2_prompt_sent: bool
    # True after ProfilerNode sends the one-time Tier 2 completion
    # message to the student. Prevents repeat pinging.
    # Default: False

    turn_count: int
    # Incremented by SupervisorNode on every turn.
    # ProfilerNode sends Tier 2 prompt only after turn_count > 2.
    # Default: 0

# All 20 fields above are checkpointed by AsyncPostgresSaver.
# (12 original fields + 8 new fields for tier-aware assessment system)
```
...
- *"My budget increased to 80,000"* → `profile_update` → ProfilerNode → updates `active_constraints` → route_after_profiler auto-reruns pipeline → updated cards emitted immediately
...
## 5b. route_after_profiler() — Conditional Edge on Profiler Node

A second conditional edge function, registered on the profiler node. Fires after every ProfilerNode execution.

```python
def route_after_profiler(state: AgentState) -> str:
    """
    Conditional edge after ProfilerNode.
    After a profile_update with profiling complete:
    automatically reruns the recommendation pipeline so
    cards update immediately without the student having
    to explicitly ask again.
    All other cases: END as before.
    """
    if (
...
**WHY THIS EXISTS:**
Without this edge, when a student updates a constraint mid-chat (e.g. "change my budget to 40,000"), ProfilerNode acknowledges the update but the pipeline never reruns. The student sees stale cards until they explicitly ask for recommendations again.

With this edge, any profile_update on a complete profile automatically triggers a fresh pipeline run: FilterNode → ScoringNode → ExplanationNode emits updated cards, and saves a new DB snapshot with trigger="profile_update".

**WHAT DOES NOT CHANGE:**
Initial profiling (profiling_complete=False) always routes to END. route_by_intent() is unchanged. All other routing is unchanged.

**KEY ROUTING RULE (preserved from v2.1):**
If get_recommendation intent is detected but profiling_complete is False, the conditional edge in route_by_intent overrides to profiler instead. This rule is unchanged.

**PATH A — Initial profiling (END, no auto-rerun):**

  Student: "what should I study?" produces get_recommendation
  route_by_intent: profiling_complete=False routes to profiler
  ProfilerNode: asks for budget
  route_after_profiler: last_intent is get_recommendation, not
    profile_update, so returns END
  Result: student asked for budget. Correct.

**PATH B — Profile update, profiling already complete:**

  Student: "change budget to 40,000" produces profile_update
  route_by_intent: routes to profiler
  ProfilerNode: updates active_constraints.budget to 40000
  route_after_profiler: last_intent is profile_update AND
    profiling_complete is True, so returns filter_node
  FilterNode runs with new budget, ScoringNode, ExplanationNode
  Updated cards emitted, DB snapshot saved with trigger=profile_update
  Result: updated cards shown immediately. Correct.
...
4. Return state — route_after_profiler handles the rest automatically
5. If profiling_complete=True: full pipeline reruns via route_after_profiler → updated cards emitted immediately (no explicit confirmation step needed)
...
## 6b. Tier-Aware Counsellor Strategy

ProfilerNode selects one of three counsellor strategies at the start of each execution based on which assessment data is present in state.

```python
def _select_counsellor_strategy(state: AgentState) -> str:
    has_caas = bool(state.get("caas_scores"))
    has_kcis = bool(state.get("kcis_scores"))
    has_vna  = bool(state.get("vna_scores"))

    tier2_complete = has_caas and has_kcis
    tier3_complete = has_vna

    if not tier2_complete:
        return "tier1_compensatory"
    elif not tier3_complete:
        return "tier2_standard"
    else:
        return "tier3_enriched"
```

**STRATEGY tier1_compensatory:**
Fires when caas_scores or kcis_scores are absent (Tier 2 not done).

Behaviour:
- After turn_count > 2 AND tier2_prompt_sent is False: insert the one-time Tier 2 completion message. Sets tier2_prompt_sent to True. Never repeats.
- If student declines the Tier 2 prompt: proceed with Tier 1 data.
- Asks proxy questions conversationally, one per turn:
  - "Have you thought much about what you want to do after school?" (proxies CAAS Concern)
  - "Within your top interest type, what activities appeal most?" (proxies KCIS sub-scale discrimination)
  - "Is this entirely your own decision or are family expectations involved?" (proxies CAAS Cooperation and family_context)
- Does NOT produce degree recommendations. ExplanationNode's role.

**STRATEGY tier2_standard:**
Fires when caas_scores AND kcis_scores are both present.

Stage selection using CAAS Concern subscale:
```python
concern = state["caas_scores"].get("concern", 3.0)
if concern < 2.5:   stage = "early"    # not thinking about future, explore
elif concern < 3.5: stage = "mid"      # some awareness, clarify and guide
else:               stage = "late"     # ready to decide, be directive
```

Family pressure detection:
- Reads caas_scores["cooperation"] and family_context.
- Low cooperation AND family_career_expectation is expects_specific_field triggers family-negotiation counsellor framing immediately.
- Does not wait for student to raise it.

KCIS-informed questioning:
- Reads kcis_scores to reference specific sub-fields in questions.
- Example: "You're drawn to mathematical/computational work within your Investigative interests — this points more toward CS and Data Science than Biomedical or pure sciences."

**STRATEGY tier3_enriched:**
Fires when caas_scores, kcis_scores, AND vna_scores all present.
All tier2_standard behaviours apply, plus:

Values-informed prestige computation:
- Reads vna_scores["social_status"] to compute prestige_preference.
- Reads vna_scores["independence"] to flag structured vs entrepreneurial degree path preference.

Big Five signals (when present in state):
- Low conscientiousness + high-workload degree (MBBS, Law, Engineering): ProfilerNode raises this before recommendations: "This degree demands sustained effort over 5 years. How do you typically handle long, difficult projects?"
- High neuroticism + high-pressure degree: "This path involves significant sustained pressure. Let's talk about how you manage during difficult periods."

---

## 6c. Family Context — Step 4 Screen, Not Conversational

family_context is written from the Step 4 Preferences screen. PROFILER_REQUIRED_FIELDS remains empty list []. ProfilerNode reads family_context from state at session start. No conversational extraction for family_career_field or family_career_expectation.

The ONE remaining conversational extraction:
- social_pressure_field: triggered ONLY when family_career_expectation equals "expects_specific_field".
- ProfilerNode asks exactly one question per session.
- Answer stored in family_context["social_pressure_field"].
...
After ExplanationNode responds, questions route to AnswerNode via SupervisorNode — not back through the full pipeline. Full pipeline reruns when:
- Student explicitly asks for fresh recommendations (get_recommendation)
- Student updates their profile (profile_update + profiling_complete=True → route_after_profiler auto-reruns)
...
def build_graph(checkpointer: AsyncPostgresSaver) -> StateGraph:
    ...
    # Profiler: conditional — profile_update + profiling_complete
    # True routes to filter_node (auto-rerun). All else routes to END.
...
# One row per pipeline run per student
{
    ...
    roadmap_snapshot: JSONB  # list of 20-field university card entries
    trigger:          str    # "initial" | "profile_update" | "manual_rerun"
}
```

**trigger field semantics as implemented in chat.py:**
```
"initial"        — first run, no prior roadmap existed
"profile_update" — pipeline auto-ran via route_after_profiler
                   after a profile_update intent
"manual_rerun"   — student explicitly asked for recommendations,
                   prior roadmap existed
```
NOTE: "profile_update" is now active — not dead code.

Implementation in `_write_recommendation()`:
```python
trigger=(
    "initial"
...
**University cards (rich_ui events) emitted when last_intent is:**
- `"get_recommendation"` — first or explicit recommendation request
- `"follow_up"` — student references existing recommendations
- `"profile_update"` — profile_update auto-rerun completed

Roadmap timeline emitted ONLY on `"get_recommendation"`. Not emitted on follow_up or profile_update. The timeline does not change on reruns — only cards change.
...
| route_after_profiler | Conditional edge on profiler node. profile_update AND profiling_complete=True routes to filter_node for auto-rerun. All other cases route to END. Implemented in core_graph.py. | Without this edge, profile constraint updates never re-ran the pipeline. Students saw stale cards until explicitly asking again. |
| profile_update auto-rerun | When student updates any constraint mid-chat with complete profile, full pipeline reruns automatically. Student sees updated cards without asking explicitly. Cards emitted, DB snapshot saved. | Eliminates the "I changed my budget but cards didn't update" UX failure. |
| trigger field values | "initial", "profile_update", "manual_rerun". profile_update is now active and not dead code. | Three real trigger states with distinct semantics. |
| cards emit intents | get_recommendation, follow_up, and profile_update all emit cards. Roadmap timeline emits on get_recommendation only. | profile_update auto-rerun produces fresh cards; timeline path is unchanged. |
| AgentState field count | 20 fields (was 12). 8 new fields: aptitude_scores, caas_scores, kcis_scores, vna_scores, family_context, prestige_preference, tier2_prompt_sent, turn_count. | Tier-aware assessment system requires richer state for counsellor strategy selection. |
| tier-aware ProfilerNode | Three strategies: tier1_compensatory (no Tier 2 data), tier2_standard (caas_scores and kcis_scores present), tier3_enriched (all tiers present). Selected at start of each ProfilerNode execution from state. | Different assessment data availability requires fundamentally different counselling approaches. |
| family_context source | Step 4 Preferences screen only. PROFILER_REQUIRED_FIELDS stays empty []. One conversational follow-up for social_pressure_field only, triggered when expectation is expects_specific_field. | Family data collected via structured screen is more reliable and complete than conversational extraction. |
| tier2_prompt_sent | ProfilerNode sends Tier 2 completion message once per session after turn_count > 2. Sets tier2_prompt_sent=True. Never repeats. Field in AgentState. | Prevents repeated Tier 2 prompting which would feel intrusive and repetitive. |
...
*Point 2 Version 2.2 — May 2026*
*Added route_after_profiler() conditional edge (profile_update auto-rerun). Added 8 new AgentState fields for tier-aware assessment system (aptitude_scores, caas_scores, kcis_scores, vna_scores, family_context, prestige_preference, tier2_prompt_sent, turn_count). Added Section 5b (route_after_profiler paths A and B). Added Section 6b (tier-aware counsellor strategy: tier1_compensatory, tier2_standard, tier3_enriched). Added Section 6c (family_context from Step 4 screen). Updated Section 11 core_graph.py profiler edge to add_conditional_edges. Updated Section 12 trigger semantics and cards emission (now includes profile_update). Added 8 new rows to Section 16 decisions table.*
